...
function [flag, path] = Astar(MAP, start, goal, k)
%Heuristic Weight%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
weight = k*sqrt(2); %Try 1, 1.5, 2, 2.5, weight = 0 gives Djikstra algorithm
...
while(~isempty(openNodes))
    ...
    %set current node
    current = openNodes(I,:);
    plot(current(1),current(2),'o','color','y','MarkerFaceColor','y')
